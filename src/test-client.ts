...
class CanvasTestClient {
  ...
  async listAssignments(courseId: string, studentId?: string, includeSubmissionHistory: boolean = false): Promise<ToolResponse> {
    return this.callTool('list-assignments', { courseId, studentId, includeSubmissionHistory });
  }

  async listAssignmentSubmissions(courseId: string, assignmentId: string, includeComments: boolean = true): Promise<ToolResponse> {
    return this.callTool('list-assignment-submissions', { courseId, assignmentId, includeComments });
  }

  async listSections(courseId: string, includeStudentCount: boolean = false): Promise<ToolResponse> {
    return this.callTool('list-sections', { courseId, includeStudentCount });
  }

  async getRubricStatistics(courseId: string, assignmentId: string, includePointDistribution: boolean = true): Promise<ToolResponse> {
    return this.callTool('get-rubric-statistics', { courseId, assignmentId, includePointDistribution });
  }
